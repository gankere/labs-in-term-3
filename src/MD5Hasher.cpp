...
void md5_transform(uint32 state[4], const uint8 block[64]) {
    ...
    for (int i = 0; i < 16; ++i)
        x[i] = (static_cast<uint32>(block[i * 4]) |
...
void md5(const uint8* input, size_t length, uint8 digest[16]) {
    uint32 state[4] = {0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476};
    size_t total_bits = length * 8;
    size_t pad_len = (56 - (length + 1) % 64) % 64;
    size_t total_len = length + 1 + pad_len + 8;

    uint8* buffer = new uint8[total_len];
    std::memcpy(buffer, input, length);
    buffer[length] = 0x80;
    std::memset(buffer + length + 1, 0, pad_len);
    for (int i = 0; i < 8; ++i)
        buffer[total_len - 8 + i] = static_cast<uint8>((total_bits >> (i * 8)) & 0xff);

    for (size_t i = 0; i < total_len; i += 64)
        md5_transform(state, buffer + i);

    for (int i = 0; i < 4; ++i) {
        digest[i * 4 + 0] = static_cast<uint8>(state[i] & 0xff);
        digest[i * 4 + 1] = static_cast<uint8>((state[i] >> 8) & 0xff);
        digest[i * 4 + 2] = static_cast<uint8>((state[i] >> 16) & 0xff);
        digest[i * 4 + 3] = static_cast<uint8>((state[i] >> 24) & 0xff);
    }
...
public:
    std::string hash(const char* data, size_t size) const override {
        uint8 digest[16];
        md5(reinterpret_cast<const uint8*>(data), size, digest);
